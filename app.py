...
APP_VERSION = "15.2-dashboard-team-upload-performance"
...
def normalize_amazon(reco, source_path):
    ...
    for _, r in reco.iterrows():
        ...
        if "payment pending" in remark.lower():
            payment_status = "Pending"
        elif "short payment" in remark.lower():
            payment_status = "Short Payment"
        elif received <= 0 and net_billing > 0:
            payment_status = "Pending"
...
def load_tasks():
    """Load tasks with one owner-rule query and vectorized aging."""
    with db() as c:
        t = pd.read_sql_query("""
            SELECT * FROM pending_tasks
            ORDER BY task_created_date DESC, portal, order_no
        """, c)
    ...
    is_completed = (
        t["task_status"].fillna("").astype(str).eq("Completed")
        & completed_dates.notna()
    )
    end_dates = pd.Series(today, index=t.index)
    end_dates.loc[is_completed] = completed_dates.loc[is_completed]
    t["aging_days"] = (
        (end_dates - start_dates).dt.days.fillna(0).clip(lower=0).astype(int)
    )

    # Load owner rules once, then map entirely in memory.
    rules = load_owner_rules()
    owner_map = {}
    fallback_map = {}
    if rules is not None and not rules.empty:
        for _, rr in rules.iterrows():
            branch = txt(rr.get("branch_code"))
            task_type = txt(rr.get("task_type"))
            name = txt(rr.get("owner_name"))
            email = txt(rr.get("owner_email"))
            key = (branch, task_type)
            target = fallback_map if branch.lower() == "all branch" else owner_map
            target.setdefault(key if target is owner_map else task_type, {"names": [], "emails": []})
            bucket = target[key if target is owner_map else task_type]
    ...
    assignees, emails = [], []
    for branch, task_type in zip(
        t["branch_code"].fillna("").astype(str),
        t["task_type"].fillna("").astype(str),
    ):
        bucket = owner_map.get((branch.strip(), task_type.strip()))
        if not bucket:
            bucket = fallback_map.get(task_type.strip())
        if bucket:
            assignees.append(" | ".join(bucket["names"]))
            emails.append(" | ".join(bucket["emails"]))
        else:
            assignees.append("")
            emails.append("")
...
def load_mir():
    with db() as c:
        m = pd.read_sql_query("""
            ...
            ORDER BY branch_code,updated_at DESC,order_no
        """, c)
        ...
        for _, r in df.iterrows():
            ...
            c.execute("""
                INSERT INTO mir_details(
                    ...
                    product_code,mir_no,mir_date,tei_no,tei_date,sr_no,sr_date,
            ...
            if txt(r.get("TEI No")) and iso_date(r.get("TEI Date")):
                c.execute("""
                    ...
                    SET task_status='Completed',
                        task_completed_date=COALESCE(task_completed_date,?),
                        team_remarks=CASE
                            WHEN COALESCE(team_remarks,'')='' THEN 'TEI updated by branch team'
                            ELSE team_remarks
                        END,
                        last_update=?
            ...
            c.execute("""
                ...
                VALUES(?,?,?,?,?,?,?,?)
            """,(now,"MIR UPDATE","",str(branch),order_no,"MIR/TEI","",f"MIR={txt(r.get('MIR No'))}; TEI={txt(r.get('TEI No'))}; SR={txt(r.get('SR No'))}"))
            count += 1
...
def load_master():
    ...
    with db() as c:
        return pd.read_sql_query("""
            ...
            ORDER BY source_uploaded_at DESC, portal, order_no
        """, c)
...
def _master_with_operational_overlay(master):
    ...
    if not mir.empty:
        ...
        if missing_mir.any():
            ...
            for col in [
                "mir_no","mir_date","tei_no","tei_date","sr_no","sr_date","mir_remarks"
            ]:
                if col in fallback.columns:
                    master.loc[missing_mir,col] = fallback[col].values
    else:
        for c in [
            "mir_no","mir_date","tei_no","tei_date",
...
def ecom_process_display(master):
    ...
    derived_task_type = master["pending_remarks"].fillna("").astype(str).apply(
        task_type_from_remark
    )
    out["Task Type"] = task_type_series.where(
        task_type_series.str.strip() != "",
        derived_task_type
    )

    # Exact attached visible output order.
    ...
    out["Courier & Customer Return Qty"] = master.get("courier_customer_return_qty",0)
    ...
    active_issue = out["Task Type"].fillna("").astype(str).str.strip() != ""
    out["Task Status"] = raw_task_status.where(
        raw_task_status.str.strip() != "",
...
def apply_global_filters(df, prefix):
    ...
    if "Task Type" in df.columns:
        for value in df["Task Type"].fillna("").astype(str):
            for part in value.split("|"):
                part = part.strip()
                if part:
                    detected_tasks.append(part)

    if "Pending Remarks" in df.columns:
        for value in df["Pending Remarks"].fillna("").astype(str):
            task = task_type_from_remark(value)
            if task:
    ...
    # Always expose business task types; detected task types are added as well.
    task_options = list(dict.fromkeys(
        canonical_tasks + sorted(set(detected_tasks))
    ))
    ...
    if pt:
        if "Task Type" in view.columns:
            view = view[view["Task Type"].fillna("").astype(str).apply(
                lambda s: any(x in s for x in pt)
            )]
        elif "Pending Remarks" in view.columns:
            view = view[view["Pending Remarks"].fillna("").astype(str).apply(
                lambda s: task_type_from_remark(s) in pt
            )]
...
def apply_branch_team_workbook(uploaded, branch):
    """
    Fast branch-team upload for Supabase/PostgreSQL.

    Reads current task/MIR state in bulk, computes changes in memory, then performs
    batched writes inside one database transaction. Blank cells never clear saved
    team values, unchanged re-uploads are no-ops, and completed tasks stay completed.
    """
    ...
    df = df.copy()
    df["__order__"] = df["Order No"].map(clean_id)
    df["__branch__"] = df["Branch Code"].fillna("").astype(str).str.strip()
    df = df[df["__order__"].ne("")].copy()

    wrong_branch = df[df["__branch__"].ne(str(branch))]
    ...
    order_nos = list(dict.fromkeys(df["__order__"].tolist()))
    if not order_nos:
        return 0, []
    ...
    def changed_text(new_value, old_value):
        return bool(txt(new_value)) and txt(new_value) != txt(old_value)

    def changed_date(new_value, old_value):
        new_iso = iso_date(new_value)
        return bool(new_iso) and new_iso != iso_date(old_value)

    # Resolve targets from System Mapping in memory.
    targets_by_order = {}
    if not mapping.empty and {"Order No", "Branch Code"}.issubset(mapping.columns):
        mm = mapping.copy()
        mm["__order__"] = mm["Order No"].map(clean_id)
        mm["__branch__"] = mm["Branch Code"].fillna("").astype(str).str.strip()
        mm = mm[(mm["__order__"].isin(order_nos)) & (mm["__branch__"].eq(str(branch)))]
        for _, mr in mm.iterrows():
            oid = mr["__order__"]
            portal = txt(mr.get("Market Place"))
            task_type = txt(mr.get("Task Type"))
            if portal and task_type:
                targets_by_order.setdefault(oid, []).append((portal, oid, task_type))

    placeholders = ",".join("?" for _ in order_nos)

    with db() as c:
        # One fallback task query for all uploaded orders.
        task_rows = c.execute(
            f"""
            SELECT portal,order_no,task_type,task_status,working_date,
                   task_completed_date,team_remarks,ticket_raised,raised_date
            FROM pending_tasks
            WHERE branch_code=? AND order_no IN ({placeholders})
            """,
            [str(branch)] + order_nos
        ).fetchall()

        current_tasks = {}
        for row in task_rows:
            portal, oid, task_type = txt(row[0]), clean_id(row[1]), txt(row[2])
            current_tasks[(portal, oid, task_type)] = {
                "status": txt(row[3]),
                "working": row[4],
                "completed": row[5],
                "remarks": row[6],
            ...
            targets_by_order.setdefault(oid, [])
            target = (portal, oid, task_type)
            if target not in targets_by_order[oid]:
                targets_by_order[oid].append(target)

        # One MIR query for all uploaded orders.
        mir_rows = c.execute(
            f"""
            SELECT order_no,mir_no,mir_date,tei_no,tei_date
            FROM mir_details
            WHERE branch_code=? AND order_no IN ({placeholders})
            """,
            [str(branch)] + order_nos
        ...
        current_mir = {
            clean_id(r[0]): {
                "mir_no": r[1], "mir_date": r[2],
                "tei_no": r[3], "tei_date": r[4],
            }
            for r in mir_rows
        }
        ...
        for _, r in df.iterrows():
            order_no = r["__order__"]
            targets = targets_by_order.get(order_no, [])
            if not targets:
            ...
            old_mir = current_mir.get(order_no, {})
            old_mir_no = old_mir.get("mir_no", "")
            old_mir_date = old_mir.get("mir_date")
            old_tei_no = old_mir.get("tei_no", "")
            ...
            if mir_changed:
                merged_mir_no = mir_no or txt(old_mir_no)
                merged_mir_date = mir_date or iso_date(old_mir_date)
                merged_tei_no = tei_no or txt(old_tei_no)
                merged_tei_date = tei_date or iso_date(old_tei_date)
                mir_upserts.append((
                    str(branch), order_no,
                    merged_mir_no, merged_mir_date,
                    merged_tei_no, merged_tei_date,
                    MIR_BRANCH_PEOPLE[str(branch)], now,
                ))
                current_mir[order_no] = {
                    "mir_no": merged_mir_no, "mir_date": merged_mir_date,
                    "tei_no": merged_tei_no, "tei_date": merged_tei_date,
                }

            task_updates_this_row = 0
            for portal, oid, task_type in targets:
                current = current_tasks.get((portal, oid, task_type))
                if not current:
                    continue

                task_changed = any([
                    changed_text(team_remarks, current["remarks"]),
                    changed_text(ticket, current["ticket"]),
                    changed_date(ticket_raised_date, current["raised"]),
                    changed_date(completed_date, current["completed"]),
                ])
                ...
                new_status = (
                    "Completed"
                    if completed_date or current["status"] == "Completed"
                    else "Working"
                )
                ...
                task_updates.append((
                    new_status,
                    new_status, today,
                    new_status, completed_date, today,
                    team_remarks, team_remarks,
                    ticket, ticket,
                    ticket_raised_date, ticket_raised_date,
                    now, portal, oid, task_type,
                ))

                audit_rows.append((
                    now, "BRANCH TEAM UPDATE", portal, str(branch),
                    oid, task_type, uploaded.name,
                    (
                        f"MIR={mir_no}; MIR Date={mir_date}; "
                        f"TEI={tei_no}; TEI Date={tei_date}; "
                        f"Ticket={ticket}; Ticket Date={ticket_raised_date}; "
                        f"Remarks={team_remarks}; Completed={completed_date}; "
                        f"Status={new_status}"
                    ),
                ))
    ...
    invalidate_read_cache()
    return updated, list(dict.fromkeys(unmatched))
...
backfill_missing_tasks_from_master()

# ============================================================
# UI
# ============================================================
st.title("E-Commerce Reconciliation Control Tower")
st.caption("Build: v14.28 — Persistent Source + Team History")
st.caption(
...
# ------------------------------------------------------------
if workspace == "E-Com Process Reconciliation Dashboard":
    ...
    st.subheader("E-Com Process Reconciliation Dashboard")
    st.caption(
        "Format follows the attached E-Com Process Reco sheet. "
    ...
    st.download_button(
        "Download Filtered E-Com Process Reconciliation",
...
elif workspace == "Settlement Process Reconciliation Dashboard":
    st.subheader("Settlement Process Reconciliation Dashboard")
    st.caption(
    ...
    st.download_button(
        "Download Filtered Settlement Process Reconciliation",
    ...
    st.info("Download the filtered Pending Task workbook here. Only MIR No, MIR Date, TEI No, TEI Date, Ticket Raised If Any, Ticket Raised Date, Team Remarks and Task Completed Date are editable. Upload the completed workbook under the corresponding MIR branch.")
...
# ------------------------------------------------------------
    ...
    st.write(
        ...
        "The team can edit only MIR No, MIR Date, TEI No, TEI Date, "
    ...
    st.caption(
        ...
        "Editable columns: MIR No, MIR Date, TEI No, TEI Date, "
        "Ticket Raised If Any, Ticket Raised Date, Team Remarks and "
    ...
    if branch_team_file and st.button(
        ...
        try:
            ...
            st.success(
                f"{n} task record(s) updated permanently for branch {branch}. "
                "Main reconciliation has been updated."
            )
