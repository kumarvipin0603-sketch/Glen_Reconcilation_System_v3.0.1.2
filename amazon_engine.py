...
def get_remarks(row: pd.Series) -> str:
    ...
    # No billing, CN, payment, or refund activity. Keep pending only through
    # 20 calendar days from shipment; from day 21 onward mark reconciled.
    if (
        not has_sale
        and not has_sales_return
        ...
        shipment_date = pd.to_datetime(
            row.get("Shipment Date"), errors="coerce", utc=True
        )
        ...
        shipment_day = shipment_date.normalize()
        age_days = max((today_utc - shipment_day).days, 0)
